...
class TestInterviewEngine:
    ...
    def test_engine_initialization(self, db: Session, test_user: User):
        ...
        session = InterviewSession(
            user_id=test_user.id, track="swe_intern", company_style="general", difficulty="easy", status="created"
        )
        db.add(session)
        db.commit()
    ...
    def test_select_warmup_question(self, db: Session, test_user: User, sample_questions):
        ...
        session = InterviewSession(
            user_id=test_user.id, track="swe_intern", company_style="general", difficulty="easy", status="warmup"
        )
        db.add(session)
        db.commit()
    ...
    def test_select_behavioral_question(self, db: Session, test_user: User, sample_questions):
        ...
        session = InterviewSession(
            user_id=test_user.id, track="swe_intern", company_style="general", difficulty="easy", status="active"
        )
        db.add(session)
        db.commit()
    ...
    def test_select_technical_question(self, db: Session, test_user: User, sample_questions):
        ...
        session = InterviewSession(
            user_id=test_user.id, track="swe_intern", company_style="general", difficulty="medium", status="active"
        )
        db.add(session)
        db.commit()
    ...
    def test_company_style_filtering(self, db: Session, test_user: User):
        ...
        session = InterviewSession(
            user_id=test_user.id, track="swe_intern", company_style="google", difficulty="medium", status="active"
        )
        db.add(session)
        db.commit()
    ...
    def test_followup_generation(self, db: Session, test_user: User, sample_questions):
        ...
        with patch.object(engine, "llm_client") as mock_llm:
            mock_llm.chat_completion.return_value = {"follow_up": "Can you optimize this solution?"}

            followup = engine.generate_followup(question=sample_questions[0], user_answer="My answer here")
